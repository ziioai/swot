...
interface YourResponse {
  analyze0: string;  // 你对这些真题的总体分析。
  analyze1: string;  // 你认为目前笔记的总体思路是否出现重大偏颇，导致需要大幅度修改。
  analyze2: string;  // 你认为题目的特点是否有必要创建新的题型。
  analyze3: string;  // 你是否发觉到目前的笔记中有某些内容是错误的，或者是需要修改的。
  analyze4: string;  // 你是否觉得出题人的思维方式比较古怪，甚至有无可能是出题人搞错了。
  analyze5: string;  // 你觉得题目各个部分的表述是否容易引人误解（有的话需要在tips记录）。
  analyze6: string;  // 你觉得各题型的steps对于解决该类题型的问题是否具有通用性。
  analyze7: string;  // 你觉得各题型的datums和tips中是否存在过于特例或武断的内容。
  analyze8: string;  // 你对目前笔记（包括steps、datums、tips）的其他可改进之处（包括增补、除冗、除错等）的概要分析。
  analyze9: string;  // 你认为目前的笔记是否过于宽泛或过于具体，是否能帮助答题者举一反三。
  analyze10: string;  // 你的总结。
  operations: {
...
\`\`\`TypeScript
interface YourResponse {
  analyze: string;  // 根据笔记中的解题步骤（steps）、缓存数据（datums）提示（tips）对当前题目所做的初步分析，可以详细，可以很长，但不要在这个分析中对各步骤的结果做出武断判断。
  stepResults: string[];  // 你对当前题目的解题过程，根据每个步骤的要求，描述vars字典的变化。
  answer: string;  // 你对当前题目的最终回答，通常是一个字符串。
}
\`\`\`
`.trim();
...
export function SpaCE_Step3_MakeErrorCase(dataWrap: any) {
  ...
  dataWrap.errorCase.wrongStepResults = dataWrap.errorItem?.stepResults;
}
...
export const SpaCE_Demo_Data = SpaCE_Demo_Data_1;
