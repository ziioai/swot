...
export const promptVersion = "伪代码测试版（v1.4）";
...
interface QTEntry {
  ...
  tips: string[];  // 重要提示，记录你认为在解这类题时尤为需要注意的事项，越靠前的越重要。注意：tips应该相对普适和通用，而特例化的内容应该以datums的形式记录。
}
interface KDBDatum {
...
interface YourResponse {
  ...
  typeMatched: string;  // 分析：现有题型描述与当前题目是否足够匹配
  errorAnalyze: string;  // 你对错误答案的分析，描述你认为错误的原因和改进方向。
  noteAnalyze: string;  // 你对目前笔记可改进之处的分析，包括增补、除冗、除错等。

  wrongChosenType: string;  // 分析：是不是因为选择了错误的题型导致的错误（需要通过改进题型的name、desc或clue）

  unclearQuestion: string;  // 分析：是不是因为题目的表述不够清楚导致的错误（需要通过完善笔记来提示后人）
  ambiguousQuestion: string;  // 分析：是不是因为题目的表述存在歧义导致的错误（需要通过完善笔记来提示后人）

  dogmaticNote: string;  // 分析：笔记的描述是否过于死板，导致无法灵活应对题目变化（需要做相应的修改）
  redundantNote: string;  // 分析：笔记是否存在冗余内容，比如含义上的重复，导致抓不住重点（需要做相应的修改）

  tooBoardQType: string;  // 分析：现有题型是否过于宽泛，导致混淆了不同的情形，比如clue太依赖字面形式，而没有根据题目实际内容来区分（需要创建新的题型，并修改旧题型的name、desc或clue以作区分）

  weirdExplain: string;  // 分析：题目正确答案的解释是否令你不解
  newTypeNeeded: string;  // 分析：是否需要创建新的题型
