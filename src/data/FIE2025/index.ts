...
#### 字段含义
- predicate：谓词。谓词中大部分为动词，少部分为形容词。人造语料集涉及77个谓词，真实语料集涉及71个谓词。
- text：背景句（主蕴含句）。此字段提供推理所需的语境，模型需要以此为依据来判断结论句的真值情况。
- hypothesis：结论句（被蕴含句）。此字段提供推理所需的鉴别式，模型需要以背景句的内容来判断此句的真值情况。
`.trim();
export const FIE2025_instruction = `
假设text字段的内容为真，要求对hypothesis字段的内容进行分析和评估，最后回答hypothesis字段的内容是否为真。
建议从以下角度分析：
- 极性：肯定|否定|……
- 叙述情态：陈述|预言|计划|声明|承诺|……
- 事件状态：已发生|将发生|正在发生|未发生|不会发生|……
- 超命题要素（多）：正当性|应然性|道德性|社会称许性|……
- 命题主题（多）：行为表象|物理表象|心理活动|属性or身份or角色|主观偏好评价|主观价值判断|阵营立场|……
- 其他：频率、时间范围、地理范围、社会文化、人设、情绪价值、艺术性、修辞手法、因果关系、条件假设……
- 其他任何你觉得可能影响最终判断的因素。
最后回答hypothesis字段的内容是否为真。
答案仅限“真”“假”或“不能确定”，不能回答其他内容。
`.trim();
...
export const FIE2025_Training_Data_Standardized = FIE2025_Training_samples.map((it: any)=>{
  ...
  const content_ = _.pick(it, [
    // "d_id",
    "text", "hypothesis", "predicate",
    // "type",
  ]) as any;
