...
const AIModelConfigPanel = defineComponent({
  name: "AIModelConfigPanel",
  props: {
    supplierForm: {
      type: Object,
      default: null,
  ...
  emits: ['supplier-changed', 'api-key-changed', 'model-changed', 'custom-supplier-added', 'models-refreshed'],
  setup(props, { emit }) {
    ...
    const supplierForm = props.supplierForm || reactive({
      selectedSupplier: suppliers_[0] as SupplierDict,
    ...
    const availableModelOptions = computed(()=>{
      return [
        ...
        ...availableModels.value.map((model: ModelDict)=>({name: model?.label??model?.name??model?.id})),
      ];
    });
    ...
    function addSupplier() {
      ...
      emit('custom-supplier-added', _.cloneDeep(supplierToAdd));
    ...
    return ()=>{
      return [
        ...
        vnd(Panel, { header: "模型API配置", toggleable: true, class: "my-1.5rem! col" }, {
          default: () => vnd("div", {class: "stack-v"}, [
            ...
            vnd(Select, {
              ...
              "onUpdate:modelValue": (value: SupplierDict) => {
                ...
                emit('supplier-changed', _.cloneDeep(value));
              },
            }),
            ...
            vnd(InputText, {
              ...
              "onUpdate:modelValue": (value: string) => {
                if (supplierForm.selectedSupplier) {
                  ...
                  save("supplierForm", supplierForm);
                  emit('api-key-changed', { supplierName: supplierForm.selectedSupplier.name, apiKey: value });
                }
              },
            }),
              ...
              vnd(Select, {
                ...
                "onUpdate:modelValue": (value: ModelDict) => {
                  if (supplierForm.selectedSupplier) {
                    ...
                    save("supplierForm", supplierForm);
                    emit('model-changed', { supplierName: supplierForm.selectedSupplier.name, model: _.cloneDeep(value) });
                  }
                },
              }),
              vnd(ToolButton, { icon: "pi pi-refresh", /* label: "刷新", */ tooltip:"刷新模型列表", command: async ()=>{ // Changed pi-trash to pi-refresh
                if (supplierForm.selectedSupplier) {
                  await 刷新模型列表(supplierForm.selectedSupplier, supplierForm);
                  // Assuming 刷新模型列表 updates supplierForm.supplierModelsDict directly
                  const refreshedModels = supplierForm.supplierModelsDict[supplierForm.selectedSupplier.name] ?? [];
                  emit('models-refreshed', { supplierName: supplierForm.selectedSupplier.name, models: _.cloneDeep(refreshedModels) });
                } else {
                  alert("请先选择一个供应商。");
                }
              }}),
